!function($){
    ...
    function createElement(key, value, type, expanderClasses) {
        ...
        switch(type){
            case 'object':
                var object = '<li><span class="'+ expanderClasses +'"></span><span class="key">"' + encode(key) + '": </span> <span class="open-json">' + open + '</span> <ul class="' + klass + '">';
                object = object + json2html(value, expanderClasses);
                return object + '</ul><span class="close-json">' + close + '</span></li>';
                break;
            case 'number':
            case 'boolean':
